...
await page
  .locator('.fixed.inset-0[role="dialog"] > div')
  .first()
  .waitFor({ state: "visible", timeout: 10000 });
// Then check for text inside modal
await page
  .locator('[role="dialog"] >> text=AUTHORIZATION REQUIRED')
  .first()
...
await page
  .getByRole("heading", { name: "GRANTED", level: 2 })
  .last()
...
for (let i = 0; i < 8; i++) {
  ...
  if (state === "cinematic") {
    const hasWhiteBox = await page.evaluate(
      () => !!document.querySelector('[class*="white"], [style*="bg-white"]'),
    );
...
const inCinematic = await page.evaluate(() => {
  ...
  const cinematicOverlay = document.querySelector(
    '.fixed.inset-0.z-50, [style*="fixed"][style*="inset-0"][style*="z-50"]',
  );
  // Also check for video element (present in video phase)
  const video = document.querySelector("video");
  // Check for white box (done phase)
  const whiteBox = document.querySelector(
    '[class*="white"], [style*="bg-white"]',
  );
  // Check for lattice grid (lattice phase)
  const latticeGrid = document.querySelector(
    '[class*="lattice"], [class*="grid"], [style*="linear-gradient"]',
  );
  return {
    hasVideo: !!video,
    hasCinematicOverlay: !!cinematicOverlay,
    hasLatticeGrid: !!latticeGrid,
    hasWhiteBox: !!whiteBox,
    videoSrc: video?.src || "none",
  };
});
